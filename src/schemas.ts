...
export const imageUrlSchema = z
  ...
  .regex(
    ...
    "Expected a numbered WebP image path.",
  );
...
export const categoryRawSchema = z
  .object({
    ...
    icon: z
      ...
      .min(3)
      .max(64)
      .regex(/^[a-z][a-z0-9-]*$/, "Expected a kebab-case icon name."),
    description: z.string().trim().min(20).max(400),
  })
  .strict();

export const categorySchema = categoryRawSchema
  ...
  .superRefine(({ name, slug }, context) => {
    if (slug !== slugify(name))
      context.addIssue({
        code: "custom",
        path: ["slug"],
        message: "Slug must match the category name.",
      });
  });

const templateLinksSchema = z
  .object({
...
export const templateVariantRawSchema = z
  .object({
    name: nameSchema,
  ...
  .strict()
  .superRefine(({ createdAt, lastUpdate }, context) => {
    if (createdAt > lastUpdate)
      context.addIssue({
        code: "custom",
        path: ["lastUpdate"],
        message: "lastUpdate must be on or after createdAt.",
      });
  });
...
const logoUrlSchema = z
  .string()
  .regex(
    /^\/images\/[a-z0-9]+(?:-[a-z0-9]+)*\/logo\.webp$/,
    "Expected a WebP logo path.",
  );
const variantLogoUrlSchema = z
  .string()
  .regex(
    /^\/images\/[a-z0-9]+(?:-[a-z0-9]+)*\/[a-z0-9]+(?:-[a-z0-9]+)*\/logo\.webp$/,
    "Expected a WebP variant logo path.",
  );
const templateVariantFilesSchema = z
  .object({
    config: z
      .string()
      .regex(
...
export const templateVariantSchema = templateVariantRawSchema
  .safeExtend({
    slug: slugSchema,
  ...
  .superRefine(({ name, slug }, context) => {
    if (slug !== slugify(name))
      context.addIssue({
        code: "custom",
        path: ["slug"],
        message: "Slug must match the variant name.",
      });
  });

export const templateSchema = templateRawSchema
  .safeExtend({
    slug: slugSchema,
  ...
  .superRefine(({ name, slug }, context) => {
    if (!slugSchema.safeParse(slug).success || slug !== slugify(name))
      context.addIssue({
        code: "custom",
        path: ["slug"],
        message: "Slug must match the template name.",
      });
  })
  .superRefine(({ variants }, context) => {
    if (!variants.some(({ slug }) => slug === DEFAULT_VARIANT_SLUG))
      context.addIssue({
        code: "custom",
        path: ["variants"],
        message: "A default variant is required.",
      });
  });
